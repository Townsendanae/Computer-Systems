...
#include "common.h" //TODO: Add inside this .h file all BUFFs functions definitions and Buff struct 
//TODO(In common.c add all the BUFFs implementations: buf_init, sbuf_remove, sbuf_insert and Add semaphore.h library). 
...
int main(int argc, char **argv)
{
	...
	int connfd_ptr; //TODO: without pointer???
	pthread_t tid;
	int i;
	
	//TODO:worker thread0s
	// init buf (with Nworkers+1)
	sbuf_init(&sbuf, workers+1);
	...
	while (1) {
		...
		//TODO: replace all this while content with just accept() and BUFFINSERT method
	}
}
...
//	free(vargp);
	...
	//TODO:BUFF_REMOVE (while) -- view last ppt slide  (just detach and this while)
	while(1){
